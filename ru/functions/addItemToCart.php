...
if (hash_equals($verification_token, $verification_token1)) {
  ...
  if ($is_authorized) {
    ...
    if ($stmt = $mysqli->prepare("SELECT `ProductName` FROM `pricelistru` WHERE PriceListID=?")) {
    ...
    if ($stmt = $mysqli->prepare("SELECT `PriceListID`, `ProductName` FROM `pricelistru` WHERE PriceListID=?")) {
      $stmt->bind_param("i", $pricelistID);
      $stmt->execute();
