          ...
          <div class="col-md-12 pr-3">          
            ...
            <div class="h5 mb-0 px-3 py-2 float-right">Итого:</div>
              ...
              <div class="modal-footer">
                ...
                <button type="button" class="btn btn-dark" data-dismiss="modal" onclick="checkoutCart(`%2$s`, true)">Оформить заказ</button>
              </div>
            </div>
          </div>
        </div>',
  ...
  if ($result = mysqli_query($mysqli, $query)) {
    while ($row = $result->fetch_assoc()) {
      $html .= sprintf(
            ...
            <div class="card" style="height:100%%;">
              ...
              <div class="container text-center">
                <div class="row">
                  %6$s
                  <div class="col">
                    <a href="product.php?id=%1$s" class="btn btn-dark" style="margin: auto; margin-bottom:1.5rem; height:42px;">Подробнее</a>
                  </div>
                </div>
              </div>
              %5$s
        ...
        $row['ProductName'],
        (($row['Price'] != 0) ?
          ('<b class="text-center" style="margin: auto; margin-top:1rem; margin-bottom:1rem;">Цена – ' . penny_price_to_normal_price($row['Price']) . ' ₴</b>') : ''),
        (($row['ProductAvailability'] == 0) ?
          ('<p class="text-center bd-highlight" style="margin: auto; margin-bottom:1.5rem;">Нет в наличии</p>') : ''),
        (($row['ProductAvailability'] != 0) ?
          '<div class="col">
  ...
  if ($stmt = $mysqli->prepare($query)) {
    ...
    if ($stmt->fetch()) {
      ...
      $html = sprintf(
        ...
        (($imageHighQuality != null) ? ('<a href="/'. $imageHighQuality .'" target="_blank">Открыть большое изображение</a>') : '')        
      );
