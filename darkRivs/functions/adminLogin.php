...
if ($stmt = $mysqli->prepare("SELECT UserLogin FROM admin_passwords WHERE UserLogin=?")) {
...
if (hash_equals($verification_token, $verification_token1)) {
  ...
  if ($stmt = $mysqli->prepare("SELECT Nonce, Tag, HashedPassword FROM admin_passwords WHERE UserLogin=?")) {
    $stmt->bind_param("s", $email);
    $stmt->execute();
  ...
  if ($is_valid == 1) {
    ...
    $_SESSION['security_admin_token'] = $security_token;
    setcookie('security_admin_token', $security_token, time() + 60 * 60 * 24 * 30, '/', 'rivs.com.ua', false, true);
    setcookie('security_admin_token', $security_token, time() + 60 * 60 * 24 * 30, '/', '25.142.71.242', false, true);
